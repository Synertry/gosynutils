...
// Repeated calls must not collide. A generator that returns the same bytes
// twice would hand out duplicate credentials, so this is the property that
// actually matters rather than the length.
...
// An all-zero result would pass a length check while carrying no entropy.
func TestSecureBytesAreNotAllZero(t *testing.T) {
	...
	t.Error("SecureBytes() returned all zero bytes, which indicates no entropy was read")
}
...
// The token travels through URLs, form fields, headers and filenames, so it
// must contain nothing that would need escaping in any of them.
